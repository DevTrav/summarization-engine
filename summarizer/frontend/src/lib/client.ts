...
export function Environment(name: string): BaseURL {
    return `https://${name}-summarizer-c9bi.encr.app`
}
...
/**
 * Client is an API client for the summarizer-c9bi Encore application.
 */
...
export namespace monitor {
    ...
    export class ServiceClient {
        ...
        public async Check(siteID: number): Promise<void> {
            await this.baseClient.callTypedAPI("POST", `/check/${encodeURIComponent(siteID)}`)
        }
        ...
        public async CheckAll(): Promise<void> {
            await this.baseClient.callTypedAPI("POST", `/checkall`)
        }
        ...
        public async Ping(url: string[]): Promise<PingResponse> {
            ...
            const resp = await this.baseClient.callTypedAPI("POST", `/ping/${url.map(encodeURIComponent).join("/")}`)
...
export namespace site {
    ...
    export class ServiceClient {
        ...
        public async Delete(siteID: number): Promise<void> {
            await this.baseClient.callTypedAPI("DELETE", `/site/${encodeURIComponent(siteID)}`)
        }
        ...
        public async Get(siteID: number): Promise<Site> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("GET", `/site/${encodeURIComponent(siteID)}`)
            return await resp.json() as Site
        }
...
class BaseClient {
    ...
    constructor(baseURL: string, options: ClientOptions) {
        ...
        if (typeof window === "undefined") {
            this.headers["User-Agent"] = "summarizer-c9bi-Generated-TS-Client (Encore/v1.46.4)";
        }
